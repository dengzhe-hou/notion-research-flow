...
- `/start-my-day` —— 每日 arXiv 论文发现 + 3D 评分（相关性 + 新近度 + 质量）
- 自动去重（不重复入库）
- Top-N 论文由 Claude 自动生成 TL;DR

### Phase 2（规划中）
- 多源聚合：Semantic Scholar + DBLP 会议论文
- 5D 评分：新增引用量 + 社交信号（GitHub stars、Twitter 热度）
- `/conf-papers` —— 会议论文追踪

### Phase 3（规划中）
- `/paper-analyze` —— 深度论文分析 + PDF 提取
- `/paper-search` —— 搜索 Notion 论文库
- `/team-sync` —— 团队协作：分配论文、讨论、追踪进度
- `/weekly-digest` —— 自动生成每周摘要
