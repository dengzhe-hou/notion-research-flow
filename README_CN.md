...
./setup.sh    # 安装依赖、配置 MCP、创建 config.yaml

# 2. 编辑 config.yaml，填入你的研究方向
...
**Notion MCP 显示 "Needs authentication"**

OAuth token 会定期过期，重新授权方法：
...
然后重启 Claude Code，执行任意 Notion 命令即可触发浏览器重新授权。

**Skills 找不到（如 `/start-my-day` 无法识别）**

确保在 `notion-research-flow` 目录下运行 Claude Code。`.claude/skills/` 中的 skills 只有在工作目录为项目根目录时才会被发现。
...
**Q: 和 ARIS 是什么关系？**
A: [ARIS](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep) 覆盖全流程科研（idea → 实验 → 写作 → rebuttal），而 notion-research-flow 专注论文发现与 Notion 库管理。两者互补——用 ARIS 做科研执行，用 notion-research-flow 管理阅读。
...
├── .claude/skills/              # Claude Code 自动发现的技能
...
受 [evil-read-arxiv](https://github.com/juliye2025/evil-read-arxiv) 和 [ArxivDigest](https://github.com/AutoLLM/ArxivDigest) 启发，它们是自动化 arXiv 论文评分与发现工作流的先驱。
